...
function BorangSlot({ slot, token, partiSenarai = [], sediaAda = false, onBerjaya, onBatal }) {
        ...
        <form onSubmit={hantar} className="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-4 space-y-3">
            ...
            {sediaAda && (
                <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
                    Slot ini sudah diisi. Atas sebab privasi, butiran sedia ada tidak dipaparkan — isi semula untuk menggantikannya.
                </p>
            )}
            ...
            >
                ...
                {menghantar ? 'Menghantar…' : (sediaAda ? 'Kemaskini' : 'Hantar')}
            </button>
        </form>
    );
}
...
function BarisSlot({ slot, token, parti, aktif, onBuka, onTutup, onBerjaya }) {
    return (
        <li className="py-3 first:pt-0 last:pb-0">
            ...
            {aktif && (
                <BorangSlot slot={slot} token={token} partiSenarai={parti} sediaAda={slot.terisi} onBerjaya={onBerjaya} onBatal={onTutup} />
            )}
        </li>
    );
}
