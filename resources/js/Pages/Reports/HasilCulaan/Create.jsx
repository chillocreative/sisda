...
export default function Create({
    ...
    // Sektor Pekerjaan is a single-select field across the entire list.
    // Picking any option (including 'Lain-lain') replaces whatever was
    // previously selected. Re-clicking the current pick clears it.
    // jenis_pekerjaan_lain is wiped when the pick is anything other
    // than 'Lain-lain' so stale text never lingers in state.
    const handleSektorSingleSelect = (item) => {
        const currentValues = Array.isArray(data.jenis_pekerjaan) ? data.jenis_pekerjaan : [];
        const alreadyPicked = currentValues.length === 1 && currentValues[0] === item;
        setData(prev => ({
            ...prev,
            jenis_pekerjaan: alreadyPicked ? [] : [item],
            jenis_pekerjaan_lain: item === 'Lain-lain' && !alreadyPicked ? prev.jenis_pekerjaan_lain : '',
        }));
    };
                ...
                <div className={`rounded-xl border p-4 flex items-center justify-between ${data.is_deceased ? 'border-rose-300 bg-rose-50' : 'border-slate-200 bg-[#FFCEE3]'}`}>
                    <div>
                        <span className={`text-sm font-medium ${data.is_deceased ? 'text-rose-700' : 'text-slate-700'}`}>
                            {savingDeceased
                                ? 'Menyimpan rekod kematian…'
                                : data.is_deceased
                                    ? 'Ditandakan sebagai kematian — semua medan dikunci'
                                    : 'Tandakan sebagai kematian'}
                        </span>
                    </div>
                    <label className="flex items-center gap-2 cursor-pointer">
                        ...
                        <button
                            type="button"
                            disabled={savingDeceased || data.is_deceased}
                            onClick={() => {
                                if (data.is_deceased || savingDeceased) return;
                                if (!data.no_ic || data.no_ic.length !== 12) {
                                    alert('Sila masukkan No. IC (12 digit) dahulu sebelum menandakan sebagai kematian.');
                                    return;
                                }
                                if (!confirm('Tandakan rekod ini sebagai kematian? Rekod akan disimpan secara automatik.')) {
                                    return;
                                }
                                setData('is_deceased', true);
                                setSavingDeceased(true);
                                post(route('reports.hasil-culaan.store-deceased'), {
                                    preserveScroll: true,
                                    onError: () => {
                                        setData('is_deceased', false);
                                        setSavingDeceased(false);
                                    },
                                    onFinish: () => setSavingDeceased(false),
                                });
                            }}
                            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${data.is_deceased ? 'bg-rose-500' : 'bg-slate-300'} ${savingDeceased ? 'opacity-60 cursor-wait' : ''}`}
                        >
                ...
                <form onSubmit={handleSubmit} className={`flex flex-col space-y-6 ${data.is_deceased ? 'opacity-50 pointer-events-none select-none' : ''}`}>
                                                    ...
                                                    {group.items.map((item) => (
                                                        <label key={item} className="flex items-center space-x-2 cursor-pointer">
                                                            <input
                                                                ...
                                                                onChange={() => handleSektorSingleSelect(item)}
                                        ...
                                        <div>
                                            ...
                                            <label className="flex items-center space-x-2 cursor-pointer">
                                                <input
                                                    ...
                                                    onChange={() => handleSektorSingleSelect('Lain-lain')}
                                                    className="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500"
                                                />
                    ...
                    <div className="order-7 bg-white rounded-xl border border-slate-200 p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            {/* Status permohonan + jumlah (Dipohon / Dilulus / Dibayar) */}
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">Status Permohonan</label>
                                <select
                                    value={data.status_sumbangan}
                                    onChange={(e) => setData('status_sumbangan', e.target.value)}
                                    className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-slate-400 focus:border-slate-400"
                                >
                                    <option value="">— Pilih Status —</option>
                                ...
                                {errors.status_sumbangan && <p className="text-sm text-rose-600 mt-1">{errors.status_sumbangan}</p>}
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">Tarikh Permohonan</label>
                                <input
                                    type="date"
                                    value={data.tarikh_sumbangan}
                                    onChange={(e) => setData('tarikh_sumbangan', e.target.value)}
                                    className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-slate-400 focus:border-slate-400"
                                />
                                {errors.tarikh_sumbangan && <p className="text-sm text-rose-600 mt-1">{errors.tarikh_sumbangan}</p>}
                            </div>
                            {[
                                ...
                                <div key={field}>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input
                                            type="text"
                                            inputMode="decimal"
                                            value={formatCurrency(data[field])}
                                            onChange={(e) => {
                                                const raw = e.target.value.replace(/[^0-9.]/g, '');
                                                const parts = raw.split('.');
                                                setData(field, parts.length > 2 ? parts[0] + '.' + parts.slice(1).join('') : raw);
                                            }}
                                            placeholder="0.00"
                                            className="w-full pl-10 pr-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-slate-400 focus:border-slate-400"
                                        />
                                    </div>
                                    {errors[field] && <p className="text-sm text-rose-600 mt-1">{errors[field]}</p>}
                                </div>
                            ))}
